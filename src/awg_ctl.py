...
def gen_pulse(write_width, signal_width, offset):
    sample_rate = 2.5E9 #(samples/s)
    samples = 5000 #(maximum 2E9 samples in memory)
    ...
    # # t0_pulse_p = 150E-9 #(in s) center of pump pulse
    # t0_pulse_c = t0_pulse_p + tw_pulse_p + 150E-9 + delta_t0_pulse_c #(in s) center of control pulse
    # t0_pulse_s = t0_pulse_c + 27E-9 + delta_t0_pulse_s#(in s) center of signal pulse
...
def set_awg(awg, write_width, signal_width, offset):
    "Generate Pulses"
    samples, name_control, name_signal, control_ch, signal_ch, marker1 = gen_pulse(write_width, signal_width, offset)
